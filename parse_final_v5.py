...
# Default Teacher for School 3 (Agustiani)
DEFAULT_TID = 185
...
subject_map = {
    ...
    'DDPKDP': 218, 'DDPKTKJ': 222, 'KKDPIB': 222, 'KKTE': 219, 'KKTKR': 220, 'KKTSM': 221, 'KKTKJ': 222, 
    'MPPTO': 225, 'MPPTE': 225, 'MPPDPIB': 225, 'MPPTJKT': 225, 'KODING': 226, 'KKA': 226, 
    'PKL': 224, 'KK-TE': 219, 'KK-TKR': 220, 'DDPK-TO': 218, 'MPP-TO': 225,
        ...
        for jam, units in hours.items():
            ...
            for ci, cdata in enumerate(units[:24]):
                ...
                if sid == 212 and not tid:
                    if ci < 8: tid = get_tid(31) # Oferius
                    elif ci < 16: tid = get_tid(7) # Wijha
                    else: tid = get_tid(6) # Eliasa
                
                final.append({
                    'day': dname, 'slot_id': slid, 'classroom_id': cid,
                    'teacher_id': tid or DEFAULT_TID, 'subject_id': sid,
                    'group_key': f"{dname}_{jam}_{tid or DEFAULT_TID}_{sid}"
                })
