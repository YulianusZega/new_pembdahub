...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-gray-600">Analisis real-time kehadiran siswa tingkat sekolah</p>
            </div>
        </div>
            ...
            <div class="flex items-end gap-1">
                <div class="text-right">
                    ...
                    <h3 class="text-6xl font-bold leading-none">
                        <span id="live_cumulative_rate" class="transition-all duration-300">{{ $cumulativeStats['z'] > 0 ? round(($cumulativeStats['hadir'] / ($cumulativeStats['z'] * ($classroomStats->sum('students_count') ?: 1))) * 100, 1) : 0 }}</span>%
                    </h3>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6">
        ...
        <!-- Terlambat -->
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition group overflow-hidden relative">
            <div class="absolute -right-4 -top-4 w-20 h-20 bg-orange-50 rounded-full group-hover:scale-110 transition duration-500 opacity-50"></div>
            <div class="relative">
                <div class="w-10 h-10 bg-orange-100 rounded-xl flex items-center justify-center text-orange-600 mb-4">
                    <i class="fas fa-clock"></i>
                </div>
                <p class="text-gray-500 text-xs font-bold uppercase tracking-wider">Terlambat Hari Ini</p>
                <div class="flex items-end gap-2 mt-1">
                    <h3 class="text-2xl font-bold text-gray-800 transition-all duration-300" id="live_daily_terlambat">{{ number_format($dailyStats['terlambat']) }}</h3>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-8 py-6 border-b border-gray-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h3 class="text-xl font-bold text-gray-800">Peringkat Kehadiran Kumulatif Per Kelas</h3>
                ...
                <table class="w-full">
                    <thead class="bg-gray-50 border-b border-gray-100">
                        <tr class="bg-gray-50/50">
                            ...
                            <th class="px-8 py-4 text-left text-xs font-bold text-gray-500 uppercase tracking-widest">Nama Kelas</th>
                            <th class="px-8 py-4 text-right text-xs font-bold text-gray-400 uppercase tracking-widest">Hari Aktif</th>
                            <th class="px-8 py-4 text-right text-xs font-bold text-gray-500 uppercase tracking-widest">Total Siswa</th>
                    ...
                    <tbody class="divide-y divide-gray-50" id="live_classroom_tbody">
                        @forelse($classroomStats as $index => $cls)
                            <tr class="group hover:bg-gray-50 transition duration-150">
                                ...
                                <td class="px-8 py-5">
                                    <div class="font-bold text-gray-800 group-hover:text-teal-600 transition">{{ $cls->class_name }}</div>
                                ...
                                <td class="px-8 py-5 text-right font-semibold text-gray-600">{{ $cls->students_count }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-8 py-20 text-center text-gray-400">
                                    ...
                                    <p>Belum ada data absensi untuk kelas manapun di tanggal ini.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        async function fetchStatsData() {
            try {
                ...
                const terlambatPercentage = dailyTotal > 0 ? Math.round((ds.terlambat / dailyTotal) * 100 * 10) / 10 : 0;

                updateUIElement('live_daily_hadir', ds.hadir);
                updateUIElement('live_daily_hadir_percentage', hadirPercentage, true);
                updateUIElement('live_daily_terlambat', ds.terlambat);
                ...
                const activeStudentsCount = data.classroomStats.reduce((acc, c) => acc + (c.students_count || 0), 0) || 1;
        ...
        async function fetchLiveFeed() {
            try {
                ...
                let html = '';
                feed.forEach(item => {
                    const badgeClass = item.tipe === 'pulang' ? 'bg-orange-50 text-orange-700 border-orange-100' : (item.tipe === 'terlambat' ? 'bg-yellow-50 text-yellow-700 border-yellow-100' : 'bg-green-50 text-green-700 border-green-100');
