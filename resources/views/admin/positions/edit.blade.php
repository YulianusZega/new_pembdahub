...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-3xl font-bold text-gray-900">Edit Jabatan</h1>
                <p class="text-gray-600 mt-1">Perbarui informasi jabatan {{ $position->position_name }}</p>
            </div>
        </div>
    ...
    <form action="{{ route('admin.master.positions.update', $position->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="position_category" 
                        ...
                        <option value="structural" {{ old('position_category', $position->position_category) == 'structural' ? 'selected' : '' }}>Struktural</option>
                        <option value="functional" {{ old('position_category', $position->position_category) == 'functional' ? 'selected' : '' }}>Fungsional</option>
                        <option value="staff" {{ old('position_category', $position->position_category) == 'staff' ? 'selected' : '' }}>Staff</option>
                        <option value="support" {{ old('position_category', $position->position_category) == 'support' ? 'selected' : '' }}>Support</option>
                    </select>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="3" 
                        class="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" 
                        placeholder="Deskripsi tugas dan tanggung jawab...">{{ old('description', $position->description) }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="school_id" 
                        ...
                        @foreach($schools as $school)
                            <option value="{{ $school->id }}" {{ old('school_id', $position->school_id) == $school->id ? 'selected' : '' }}>
                                {{ $school->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="allowance_amount" value="{{ old('allowance_amount', $position->allowance_amount) }}" 
        ...
        <div class="flex items-center gap-4">
            <button type="submit" 
                ...
                <i class="fas fa-save mr-1"></i> Update Jabatan
            </button>
            <a href="{{ route('admin.master.positions.index', request()->query()) }}" 
