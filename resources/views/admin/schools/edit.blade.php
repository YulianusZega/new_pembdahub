...
<div class="space-y-6">
    ...
    <div class="mb-8">
        <div class="flex items-center gap-4 mb-2">
            <div class="flex items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-br from-green-500 to-green-600 shadow-lg">
                <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                </svg>
            </div>
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Edit Sekolah</h1>
                <p class="text-gray-600 mt-1">Perbarui informasi sekolah {{ $school->name }}</p>
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('admin.schools.update', $school) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="type" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" required>
                        ...
                        <option value="SMP" {{ old('type', $school->type) == 'SMP' ? 'selected' : '' }}>SMP</option>
                        <option value="SMA" {{ old('type', $school->type) == 'SMA' ? 'selected' : '' }}>SMA</option>
                        <option value="SMK" {{ old('type', $school->type) == 'SMK' ? 'selected' : '' }}>SMK</option>
                        <option value="yayasan" {{ old('type', $school->type) == 'yayasan' ? 'selected' : '' }}>Yayasan</option>
                    </select>
                </div>
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <textarea name="address" rows="3" 
                        class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" 
                        placeholder="Jalan, nomor, kelurahan, kecamatan...">{{ old('address', $school->address) }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 bg-blue-50 rounded-xl border border-blue-100">
                    ...
                    <div>
                        ...
                        <input type="text" name="latitude" value="{{ old('latitude', $school->latitude) }}" 
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" 
                            placeholder="Contoh: -1.23456789">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Longitude</label>
                        <input type="text" name="longitude" value="{{ old('longitude', $school->longitude) }}" 
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="principal_id" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
                        ...
                        <option value="{{ $teacher->id }}" {{ old('principal_id', $school->principal_id) == $teacher->id ? 'selected' : '' }}>
                            {{ $teacher->teacher_code }} - {{ $teacher->full_name }}
                        </option>
                        @endforeach
                    </select>
                    @if(isset($showAllTeachers) && $showAllTeachers)
                    <div class="mt-2 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                        <p class="text-xs text-amber-700">
                            <span class="font-semibold"><i class="fas fa-exclamation-triangle text-yellow-500 mr-1"></i> Info:</span> Belum ada guru dengan jabatan Kepala Sekolah di sekolah ini. 
                            Ketika Anda memilih guru, sistem akan <strong>otomatis menugaskan</strong> guru tersebut sebagai Kepala Sekolah.
                        </p>
                    </div>
                    @else
                    <p class="text-xs text-gray-500 mt-1"><i class="fas fa-lightbulb text-yellow-500 mr-1"></i> Pilih dari guru yang sudah ditugaskan sebagai Kepala Sekolah</p>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6 mb-6">
            ...
            <div class="flex items-center gap-3">
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ $school->is_active ? 'checked' : '' }}>
        ...
        <div class="flex items-center gap-4 justify-end bg-white rounded-2xl shadow-lg p-6">
            ...
            <button type="submit" 
                class="px-5 py-2.5 bg-gradient-to-r from-green-600 to-green-700 text-white rounded-xl font-semibold text-sm hover:from-green-700 hover:to-green-800 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all">
                <span class="flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
