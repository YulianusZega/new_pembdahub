...
@section('title', 'Unggah Karya & Materi Baru - Pembda Knowledge & Media')
    ...
    <div class="bg-white rounded-2xl p-6 md:p-8 border border-slate-100 shadow-sm">
        <form action="{{ route('guru.knowledge.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <input type="text" name="title" value="{{ old('title') }}" required placeholder="Contoh: Modul Dasar Bahasa Pemrograman Mikrokontroler & IoT" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent text-sm">
                @error('title') <p class="text-xs text-rose-500">{{ $message }}</p> @enderror
            </div>

            {{-- Category Type & Subject --}}
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6" x-data="{ category: '{{ old('category_type', 'sekolah') }}' }">
                <div class="space-y-2">
                ...
                <div class="space-y-2" x-show="category === 'sekolah'">
                    ...
                    <select name="subject_id" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500 text-sm">
                        ...
                        @foreach($subjects as $sub)
                            <option value="{{ $sub->id }}" {{ old('subject_id') == $sub->id ? 'selected' : '' }}>{{ $sub->subject_name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="space-y-4" x-data="{ type: '{{ old('type', 'document') }}' }">
                <div class="space-y-2">
                ...
                <div class="space-y-2" x-show="type === 'document' || type === 'audio' || type === 'video'">
                    <label class="block text-sm font-bold text-slate-800">Unggah File Media</label>
                    <input type="file" name="file" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-teal-50 file:text-teal-700 hover:file:bg-teal-100">
                    <p class="text-[11px] text-slate-500">Maksimal ukuran file: 20MB. Format didukung: PDF, Word, MP3, MP4.</p>
                </div>
                ...
                <div class="space-y-2" x-show="type === 'link' || type === 'video'">
                    ...
                    <input type="url" name="external_url" value="{{ old('external_url') }}" placeholder="https://www.youtube.com/watch?v=... atau https://github.com/..." class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500 text-sm">
                </div>
            </div>

            {{-- Thumbnail --}}
            <div class="space-y-2">
                <label class="block text-sm font-bold text-slate-800">Gambar Cover / Thumbnail (Opsional)</label>
                <input type="file" name="thumbnail" accept="image/jpeg,image/png,image/webp" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-slate-100 file:text-slate-700 hover:file:bg-slate-200">
            ...
            <div class="space-y-2">
                ...
                <textarea name="description" rows="4" placeholder="Tuliskan gambaran singkat tentang isi materi ini..." class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-teal-500 text-sm">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="bg-slate-50 p-5 rounded-xl border border-slate-200 space-y-4">
                ...
                <div class="flex flex-col sm:flex-row gap-6">
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_public" value="1" checked class="w-5 h-5 text-teal-600 rounded border-slate-300 focus:ring-teal-500">
                        <div>
                    ...
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="allow_download" value="1" checked class="w-5 h-5 text-teal-600 rounded border-slate-300 focus:ring-teal-500">
                        <div>
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                ...
                <button type="submit" class="px-6 py-2.5 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-xl shadow-md transition-all text-sm flex items-center gap-2">
                    <i class="fas fa-paper-plane"></i> Dipublikasikan Now (+10 Poin)
                </button>
            </div>
        </form>
    </div>
