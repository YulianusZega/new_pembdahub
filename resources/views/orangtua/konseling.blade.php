...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-2xl shadow-lg p-6">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                <a href="{{ route('orangtua.dashboard') }}" class="p-2 rounded-xl bg-gray-100 text-gray-600 hover:bg-gray-200 transition-colors">
                    <i class="fas fa-arrow-left"></i>
                </a>
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Catatan Konseling</h1>
                    <p class="text-gray-500">Riwayat bimbingan dan kasus siswa</p>
                </div>
            </div>
            <div class="flex items-center gap-3 bg-indigo-50 px-4 py-2 rounded-xl">
                ...
                <div>
                    ...
                    <p class="text-xs text-indigo-600 font-medium">{{ $classroom->class_name ?? 'Belum ada kelas' }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="space-y-6">
                            ...
                            <div class="mt-3">
                                @if($isImage)
                                    <div class="rounded-xl overflow-hidden border border-gray-200 max-w-sm">
                                        <img src="{{ Storage::url($record->attachment) }}" alt="Bukti" class="w-full h-auto">
                                    </div>
                                @elseif($isPdf)
                                    <a href="{{ Storage::url($record->attachment) }}" target="_blank" class="flex items-center gap-3 p-3 bg-red-50 rounded-xl border border-red-100 hover:bg-red-100 transition group w-fit">
                                ...
                                @else
                                    <a href="{{ Storage::url($record->attachment) }}" target="_blank" class="inline-flex items-center text-sm text-blue-600 hover:underline">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
            ...
            <p class="text-gray-500 text-sm mt-1">Belum ada data konseling atau prestasi yang tercatat untuk saat ini.</p>
        </div>
        @endforelse
    </div>
</div>
