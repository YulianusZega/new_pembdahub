    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="md:col-span-2 bg-white rounded-3xl border-2 border-gray-300 shadow-md p-6 md:p-8 space-y-5">
            ...
            <form action="{{ route('siswa.final-project.propose') }}" method="POST" class="space-y-5">
                ...
                <div>
                    <label for="abstract" class="block text-xs font-black text-gray-950 uppercase tracking-wider mb-1.5">Abstrak / Rencana Penelitian</label>
                    <textarea name="abstract" id="abstract" rows="6" required placeholder="Jelaskan secara singkat latar belakang, tujuan, metode, dan hasil yang diharapkan dari proyek/penelitian Anda..." class="w-full border-2 border-gray-300 rounded-xl px-4 py-2.5 text-xs focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 font-bold text-gray-950 transition leading-relaxed">{{ old('abstract') }}</textarea>
                    @error('abstract')
                        <p class="text-xs text-rose-600 mt-1.5 font-bold">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label class="block text-xs font-black text-gray-950 uppercase tracking-wider mb-1">Anggota Kelompok (Opsional)</label>
                    <p class="text-xs text-gray-950 leading-relaxed mb-3 font-bold">Pilih teman sekelas untuk menjadi anggota kelompok (Anda otomatis menjadi Ketua). Anda hanya bisa memilih teman yang belum tergabung di kelompok lain.</p>
                    
                    <div class="max-h-56 overflow-y-auto space-y-2 border-2 border-gray-300 rounded-xl p-3 bg-gray-50">
                        @forelse($classmates as $cm)
                            <label class="flex items-center gap-4 p-3 hover:bg-amber-50 rounded-xl cursor-pointer transition border-2 border-gray-300 hover:border-amber-600 shadow-sm hover:shadow-md">
