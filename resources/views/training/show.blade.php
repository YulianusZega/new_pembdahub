...
<style>
    ...
    .step-timeline-item.active .timeline-circle {
        @apply ring-4 ring-sky-100 bg-sky-500 text-white scale-110;
...
<!-- ═══════════════════ BACK BUTTON ═══════════════════ -->
<div class="mb-5">
    <a href="{{ route('training.index') }}" class="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 transition group">
        <div class="w-8 h-8 rounded-lg bg-gray-100 group-hover:bg-gray-200 flex items-center justify-center transition">
...
<!-- ═══════════════════ HEADER SECTION ═══════════════════ -->
<div class="relative overflow-hidden bg-gradient-to-br {{ $gradient }} rounded-3xl mb-8 p-8 text-white shadow-xl">
            ...
            <div class="flex-1 min-w-0">
                <div class="flex flex-wrap items-center gap-2 mb-2">
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-white/20 backdrop-blur-sm text-white">
                        <i class="{{ $icon }} mr-1.5 text-[10px]"></i> {{ $label }}
                    </span>
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-emerald-500/30 backdrop-blur-sm text-emerald-100 border border-emerald-500/25">
                        <i class="fas fa-check-circle mr-1.5 text-[10px]"></i> Interactive Tutorial
                    </span>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-4 text-xs text-white/80">
                    ...
                    @if($module->pdf_file)
                    <span class="flex items-center gap-1.5"><i class="fas fa-file-pdf"></i> Unduhan PDF Tersedia</span>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="lg:col-span-4 space-y-6">
        ...
        <div class="bg-white rounded-3xl shadow-lg border border-gray-100 p-6 sticky top-6">
            <h3 class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4 flex items-center justify-between">
                ...
                <span id="wizard-progress-percent" class="text-sky-600 bg-sky-50 px-2 py-0.5 rounded-md text-[10px]">0% Selesai</span>
            </h3>
            
            <!-- Radial / Bar Progress -->
            <div class="w-full bg-gray-100 rounded-full h-2 mb-6 overflow-hidden">
                <div id="wizard-progress-bar" class="bg-gradient-to-r from-sky-400 to-blue-600 h-full rounded-full transition-all duration-500" style="width: 0%"></div>
            </div>
        ...
        <div class="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden flex flex-col min-h-[500px]">
            ...
            <div class="px-8 py-4 bg-gray-50/70 border-b border-gray-100 flex items-center justify-between">
                ...
                <div class="flex items-center gap-1.5">
                    ...
                    <span class="text-[11px] font-semibold text-gray-500">Materi Interaktif</span>
                </div>
            </div>
        ...
        <div class="bg-white rounded-3xl shadow-lg border border-gray-100 overflow-hidden transition-all duration-300">
            ...
            <div class="px-8 py-5 border-b border-gray-100 flex items-center justify-between flex-wrap gap-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="font-bold text-gray-800">Dokumen PDF Resmi</h3>
                        <p class="text-xs text-gray-500">Unduh berkas materi panduan lengkap</p>
                    </div>
                </div>
                <a href="{{ route('training.download', $module) }}"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let checklistHtml = `
        ...
        <div class="mt-6 border border-gray-100 bg-gray-50/70 rounded-2xl p-6 space-y-4">
            <h3 class="text-sm font-bold text-gray-700 flex items-center gap-2 mb-2">
                <i class="fas fa-clipboard-check text-sky-500"></i> Lembar Verifikasi Mandiri
            </h3>
    `;
    
    checklistItems.forEach(item => {
        checklistHtml += `
            <label class="flex items-start gap-3 p-3 bg-white border border-gray-100 rounded-xl cursor-pointer hover:border-sky-300 transition-all select-none">
                <input type="checkbox" id="${item.id}" class="mt-1 w-4.5 h-4.5 rounded border-gray-300 text-sky-600 focus:ring-sky-500 cursor-pointer checklist-check-item">
    ...
    function renderTimeline() {
        ...
        steps.forEach((step, idx) => {
            ...
            timelineItem.className = `w-full text-left flex items-center gap-3 p-3 rounded-2xl transition-all duration-200 step-timeline-item ${isActive ? 'active bg-sky-50/50 font-semibold' : 'text-gray-500 hover:bg-gray-50'}`;
